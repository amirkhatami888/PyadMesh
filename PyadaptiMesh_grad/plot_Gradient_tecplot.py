...
def plotMESH(Mesh,path,name_plot):
    ...
    for node in Mesh.GiveJarOfNodes().GiveNodes():
        x.append(node.x)
        y.append(node.y)
        li=[element.fem_value for element in node.nearestElements]
        li_value_point.append(np.mean(li))    
    if not os.path.exists(f"{path}/plot"):
        os.makedirs(f"{path}/plot")    
    write_plt_tecplot(f"{path}/plot/gradientCountor-step{name_plot}.plt" ,name_plot,x,y,li_value_point,index_li)
